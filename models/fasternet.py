...
class Conv_recep(nn.Module):
    ...
    def __init__(self, input_channel, output_channel,
        ...
        self.recep_func = recep(size)
        self.conv = nn.Conv2d(input_channel, output_channel, kernel_size=kernel_size,stride=stride, padding=padding)
        #self.conv2 = nn.Conv2d(output_channel, output_channel, kernel_size=kernel_size,stride=stride, padding=padding)
    def forward(self, x):
        #x = nn.ReLU(inplace=True)(self.bn1(self.conv1(self.recep_func(x))))
        x = self.conv(self.recep_func(x)) + x
        #x = self.recep_func(x) + x
        return x
...
class Partial_conv3(nn.Module):
    ...
    def __init__(self, dim, n_div, forward):
        ...
        #self.conv_recep = Conv_recep(self.dim_conv3, self.dim_conv3, kernel_size=3, stride=1, padding=1,size = 2)
        if forward == 'slicing':
    ...
    def forward_slicing(self, x: Tensor) -> Tensor:
        ...
        #x[:, :self.dim_conv3, :, :] = self.conv_recep(self.partial_conv3(x[:, :self.dim_conv3, :, :]))
